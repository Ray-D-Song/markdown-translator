...
import limitCallRate from './limitCallRate'
import readlineFromStream from './readlineFromStream'
import type { ErrorStatus, SettledStatus, Status } from './status'
import combineAbortSignals from './combineAbortSignals'
import { isMessageError } from './error'
...
function configureApiCaller(options: ConfigureApiOptions) {
  ...
  const callApi: ApiCaller = async (
    ...
    try {
      ...
      const status: ErrorStatus = {
        status: 'error',
        message: isMessageError(err as Error) ? (err as Error).message : 'network error',
      }
      onStatus(status)
      return status
    }
